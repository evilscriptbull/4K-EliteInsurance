...
- [x] Sanity CMS wired up — `src/lib/sanity/` (client, GROQ queries, Portable Text helper) + `/blog`, `/blog/[slug]`, `scripts/seed-sanity-posts.mjs`. Fallback-safe (renders 0 posts without credentials), verified. **Not yet built**: an actual deployed Sanity Studio editing UI — content is currently authored via the seed script. See `docs/architecture.md` ("Blog / Sanity CMS").
