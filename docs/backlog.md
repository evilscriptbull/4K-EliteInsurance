...
- [x] Redirects implemented in `next.config.ts` (18 rules, verified with `next start` — all return 308 Permanent Redirect, the modern equivalent of 301) covering every mapped URL except `/privacypolicy`/`/termsandconditions` (destination pages don't exist yet, pending legal review — see `docs/site-audit/redirect-map.md`). **Not deployed yet** — per the map's own rule, this needs to go live at the same time as the DNS cutover from Squarespace, not before.
