...
- [x] Resend confirmation emails built — **live and verified 2026-08-22**, not just built. `src/lib/integrations/resend/client.ts` + `src/lib/notifications/emailNotify.ts` + `src/components/emails/RequestConfirmationEmail.tsx` send a customer-facing confirmation on quote/contact/claim submission, wired into all 3 form API routes alongside the GoTo SMS staff notifications. No-ops cleanly without credentials. Real end-to-end test: standalone send returned a live Resend message ID, and a real `/contact` submission in the browser sent successfully. **Domain not verified yet** — sends use Resend's sandbox sender (`onboarding@resend.dev`), which only delivers to the Resend account's own email; set `RESEND_FROM_EMAIL` once a domain is verified (SPF/DKIM) to send to real customers — steps in `.env.example`.
...
- [x] 6 new high-ROI posts drafted covering priority lines — general liability, commercial auto, boat (East TN lakes), group life, workers' comp (TN-specific), builders risk. Seeded as `status: "draft"` pending Chaz Goodin's compliance review per `guardrails.requireHumanReviewForHigherRiskRecommendations` — not live yet, need someone to flip `status` to `"published"` in Sanity once reviewed.
