...
- [x] GoTo SMS integration built — `src/lib/integrations/goto/client.ts` (real Messaging V2 API client, OAuth Authorization Code flow) + one-time setup routes (`/api/integrations/goto/oauth/{start,callback}`) + `src/lib/notifications/leadNotify.ts` (internal staff SMS on new lead/contact/claim, wired into all 3 form API routes). No-ops cleanly without credentials, verified. Owner still needs to complete the one-time OAuth setup (register redirect URI, visit `/oauth/start`) to get it live — steps in `.env.example`.
