...
A full review of the live-chat/staff-dashboard code (`tasks/todo.md`, 2026-09-24) found that every staff-auth checkpoint checked only "is there a valid Supabase session," never "is this an active, current associate." Public sign-up was disabled by the owner on 2026-09-24 (`disable_signup: true`, verified against `/auth/v1/settings`), which closes the outside registration path, but a deactivated associate — or any session obtained another way — would still have had full staff access. Phase 0 of that review closed the code-level gap, **2026-09-24**:

- `verifyStaffRequest()` (`src/lib/staff/verifyRequest.ts`) now looks up the caller's `associates` row after validating the session, and returns 403 `not-an-associate` unless it exists and `active` is `true`. It returns `{ userId, associate }` so the 5 staff routes it gates (`claim`, `release`, `complete`, `complete-setup`, `message`) don't re-fetch the associate separately.
- `/staff/dashboard` (`src/app/staff/dashboard/page.tsx`) checks the same condition before running any live-queue/follow-up queries, rendering a plain "no staff access" message with zero data fetched otherwise.
- `src/proxy.ts` stays session-only on purpose — it only decides whether to redirect to `/staff/login`; the page and the API routes are what actually enforce associate membership, so there's one source of truth for "is this person staff" rather than three.
