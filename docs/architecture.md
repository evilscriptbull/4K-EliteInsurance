...
**Not yet built:** a deployed Sanity Studio for a real editing UI (posts are currently authored via the seed script or Sanity's own Vision/manage tooling), and the restaurant-insurance worked example from `/new-page-2` — per the agency owner, that stays a structural reference for future posts rather than being published itself.
